...
export function verdictPrompt(a: Analysis, hold = false): { system: string; user: string } {
  ...
  return {
    ...
    user:
      ...
      `CALLOUT RULES — callout_text posts PUBLICLY on the coin's pump.fun page. STYLE: deadpan degen shitpost — everyone half-knows it's trolling, and the alpha hides inside the bit. SHORT (one or two short sentences, under ~140 chars, lowercase preferred), absurd confidence, zero explanation. Shapes that work: fake TA with absurdly wrong specificity ("cup and handle on the 5 second chart"), insider larp ("cabal said to trim before they turn on the faucet"), machine-quant absurdity ("backtested on 11 seconds of data. verdict: yes"), or a pure command ("rip it to bonding"). Fake indicators/timeframes/cabal bits are jokes and always allowed; REAL numbers are different — at most one, straight from the DATA (mc or the spiciest check), most lines need none. Never name another caller. BANNED: stat dumps, "solid fundamentals", hashtags, DYOR, financial advice, emoji spam (1 max).\n` +
