...
export function verdictPrompt(a: Analysis, hold = false): { system: string; user: string } {
  ...
  return {
    ...
    user:
      ...
      `CALLOUT RULES — callout_text posts PUBLICLY on the coin's pump.fun page. STYLE: deadpan degen shitpost — everyone half-knows it's trolling, and the alpha hides inside the bit. SHORT (one or two short sentences, under ~140 chars, lowercase preferred), absurd confidence, zero explanation. INVENT THE BIT FRESH — the coin's own name/ticker is the best prop (pun it, take it literally, treat it as an indicator). Shapes to build in, described not quoted: fake TA with absurdly wrong specificity on a nonsense timeframe; insider larp implying a source that obviously doesn't exist; machine-quant absurdity (your model/backtest doing something unhinged about this exact coin); or a pure command with zero justification. Never reuse a line or near-paraphrase from any earlier callout. Fake indicators/timeframes/cabal bits are jokes and always allowed; REAL numbers are different — at most one, straight from the DATA (mc or the spiciest check), most lines need none. Never name another caller. BANNED: stat dumps, "solid fundamentals", hashtags, DYOR, financial advice, emoji spam (1 max).\n` +
