...
export function verdictPrompt(a: Analysis, hold = false): { system: string; user: string } {
  ...
  return {
    ...
    user:
      ...
      `CALLOUT RULES — callout_text posts PUBLICLY on the coin's pump.fun page. It is a HOOK WITH RECEIPTS: scrollers should laugh, get curious, and CLICK — but the line MUST contain at least one concrete, checkable detail FROM THE DATA BELOW (dev's bond record, mc, holder shape, what the caller tape says — whichever is spiciest). The joke earns the click, the detail earns the follow; a callout that is all vibes and no substance is spam and gets him muted. Degen-native voice. BANNED: dry stat dumps, "solid fundamentals", hashtags, DYOR, financial advice, emoji spam (1 max). Think shitpost with receipts — never a shitpost without them.\n` +
